...
class MetaConnector(BaseConnector):
    ...
    async def publish(self, content: str, options: Optional[dict[str, Any]] = None) -> ActionResult:
        ...
        if target == "instagram" and self.ig_user_id:
            ...
            resp = await self._client.post(
                ...
                params={"access_token": self.page_token, "caption": content, "image_url": options.get("image_url", "")}
            )
            ...
            resp2 = await self._client.post(
                ...
                params={"access_token": self.page_token, "creation_id": creation_id}
            )
    ...
    @BaseConnector.with_retry()
    async def reply(self, thread_ref: str, content: str, options: Optional[dict[str, Any]] = None) -> ActionResult:
