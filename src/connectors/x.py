...
class XConnector(BaseConnector):
    def __init__(self, policy_engine: PolicyEngine):
        ...
        self._client = httpx.AsyncClient(base_url="https://api.x.com/2", headers={"Authorization": f"Bearer {self.access_token}", "Content-Type": "application/json"})
    ...
    async def publish(self, content: str, options: Optional[dict[str, Any]] = None) -> ActionResult:  # type: ignore[override]
        ...
        res_data = resp.json()
        return ActionResult(ok=True, platform=self.platform, action_type=ActionType.PUBLISH, external_id=res_data.get("data", {}).get("id"), idempotency_key=uuid.uuid4().hex, policy_decision_id=self._last_decision_id, raw_data=res_data)
    ...
    async def reply(self, thread_ref: str, content: str, options: Optional[dict[str, Any]] = None) -> ActionResult:  # type: ignore[override]
        ...
        payload = {"text": content, "reply": {"in_reply_to_tweet_id": thread_ref}}
        resp = await self._client.post("/tweets", json=payload)
        resp.raise_for_status()
        res_data = resp.json()
        return ActionResult(ok=True, platform=self.platform, action_type=ActionType.REPLY, external_id=res_data.get("data", {}).get("id"), idempotency_key=uuid.uuid4().hex, policy_decision_id=self._last_decision_id, raw_data=res_data)
