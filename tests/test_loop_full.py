...
async def test_loop_tick_complete_exception(mocker):
    ...
    mock_storage.fetch_next_event = mocker.AsyncMock(return_value={"id": "evt_crash", "event_type": "x_webhook", "payload": {"text": "crash"}})
...
async def test_loop_tick_urgent_meta(mocker):
    mock_event = {"id": "evt_urgent_2", "event_type": "meta_webhook", "payload": {"entry": [{"id": "meta_thread_1"}]}}
...
@pytest.mark.asyncio
async def test_loop_tick_action_failure(mocker, monkeypatch):
    loop = AutonomyLoop()
    monkeypatch.setattr("src.agent.strategy.settings.x_execution_mode", "api")
    monkeypatch.setattr("src.agent.strategy.settings.x_access_token", "test-token")
    mock_storage = mocker.patch("src.agent.loop.storage")
    mock_storage.fetch_next_event = mocker.AsyncMock(return_value={"id": "evt_act_fail", "event_type": "x_webhook", "payload": {"text": "urgent but fails"}})
    ...
    mocker.patch("src.agent.loop.dispatcher.execute_reply", return_value=ActionResult(ok=False, error="API Limit", platform=Platform.X, action_type=ActionType.REPLY, idempotency_key="key", policy_decision_id="none"))
